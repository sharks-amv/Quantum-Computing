"""
BB84 Quantum Key Distribution - 3D Simulation Surface
Part 1.4 Deliverable #4

Generates a 3D surface plot of QBER as a function of:
  - Eve's attack probability (0 → 1)
  - Number of qubits sent (100 → 5000)

This shows how the error rate depends on BOTH parameters simultaneously.

Output: qber_surface_3d.png
"""
...
def plot_qber_surface(n_trials=5, save=True):
    """
    Generate a 3D surface plot: QBER = f(attack_prob, n_qubits).

    Parameters
    ----------
    n_trials : int
        Number of trials to average at each grid point.
    save : bool
    ...
    # Add the theoretical plane at QBER = 0.25
    ...
    ax.set_title("BB84 Simulation: QBER Surface\n(Attack Probability × Number of Qubits)",
                 fontsize=14, fontweight='bold', pad=20)

    ax.view_init(elev=25, azim=225)  # Good viewing angle
    ax.set_zlim(0, 0.35)
    ...
    # --- Also create a 2D heatmap version ---
